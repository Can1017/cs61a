...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        return typed_word
    diff = min(word_list, key=lambda x: diff_function(typed_word, x, limit))
    return typed_word if diff_function(typed_word, diff, limit) > limit else diff
    # END PROBLEM 5
...
def furry_fixes(typed, source, limit):
    ...
    """
    # BEGIN PROBLEM 6
    def furry_fixes_helper(typed, source, limit, i):
        if i == len(typed) or i == len(source):
            return abs(len(typed) - len(source)) - limit
        if typed[i] != source[i]:
            limit -= 1
        if limit < 0:
            return 1
        return furry_fixes_helper(typed, source, limit, i + 1)
    return limit + furry_fixes_helper(typed, source, limit, 0)
    # END PROBLEM 6
...
def report_progress(typed, source, user_id, upload):
    ...
    for i in range(len(typed)):
        if typed[i] != source[i]:
            correct = False
            break
    progress = (i + 1 if correct and len(typed) > 0 else i) / len(source)
    ...
    for j in word_indices:
        ...
        for i in player_indices:
            if get_time(times, i, j) < min_time:
                min_time = times[i][j]
                min_index = i
        fastest[min_index].append(words[j])
    return fastest
    # END PROBLEM 10
