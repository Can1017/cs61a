...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if place.ant.is_container and place.ant.can_contain(self):
                ...
                assert place.ant is None, AssertionError(
    ...
    def double(self):
        ...
        # BEGIN Problem 12
        "*** YOUR CODE HERE ***"
        if self.double_damage is False:
            self.damage *= 2
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while place.is_hive is False:
            if place.bees and dis >= self.lower_bound and dis <= self.upper_bound:
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        ...
        "*** YOUR CODE HERE ***"
        if target is not None:
            slow_duration = 5  # 减速效果持续5个回合

            if hasattr(target, 'slow_duration'):
                target.slow_duration = slow_duration  # 重置减速效果的持续时间
            else:
...
class LaserAnt(ThrowerAnt):
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem EC 4
        base_damage = 2
        distance_penalty = 0.25 * distance
        shot_penalty = 0.0625 * self.insects_shot
        damage = base_damage - distance_penalty - shot_penalty
        return max(0, damage)
        # END Problem EC 4
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        gamestate -- The GameState, used to access game state information.
        """
        # if self.slowed and self.slowed_turns > 0:
        #     self.slowed_turns -= 1
        ...
        if self.scared and self.scared_turns > 0:
            self.scared_turns -= 1
