...
def two_list(vals, counts):
    ...
    """
    "*** YOUR CODE HERE ***"
    def helper(vals, counts, n):
        if n == 1 and counts[0] == 1:
            return Link(vals[0])
        if counts[0] == 0:
            return helper(vals[1:], counts[1:], n - 1)
        counts[0] -= 1
        return Link(vals[0], helper(vals, counts, n))
    return helper(vals, counts, len(vals))
