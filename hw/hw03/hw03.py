...
def digit_distance(n):
    ...
    """
    "*** YOUR CODE HERE ***"
    def distance(n, last_digit):
        if n == 0:
            return 0
        return abs(n % 10 - last_digit) + distance(n // 10, n % 10)
    return distance(n // 10, n % 10)
...
def interleaved_sum(n, odd_func, even_func):
    ...
    def odd_sum(x):
        if x > n:
            return 0
        return odd_func(x) + even_sum(x + 1)
    def even_sum(x):
        if x > n:
            return 0
        return even_func(x) + odd_sum(x + 1)
